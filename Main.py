...
def forward_feature_search(data):
    ...
    for i in range(1, data.shape[1]):  # shape return row and column
        ...
        for j in range(1, data.shape[1]):
            ...
            if j not in current_set:
                ...
                print(
                    f"Using feature (s) {sorted(feature_used)} accuracy is {round(accuracy * 100, 2)}%")  # Print what features are being used
        ...
        if not i == data.shape[1] - 1:
            print(
                f"Feature set {sorted(current_set)} was the best, accuracy is {round(best_accuracy * 100, 2)}%")  # Print the best found features at this level
        if not i == data.shape[1] - 1:
    ...
    print(
        f"Finished search!! The best feature subset is {sorted(best_accuracy_feature_set)} , which has an accuracy of {round(best_accuracy_of_program * 100, 2)}%")
    ...
    for i in range(1, data.shape[1]):  # shape return row and column
        ...
        for j in range(1, data.shape[1]):
            ...
            if j in feature_to_use:
                ...
                accuracy = leave_one_out_cross_validation(data, feature_to_use,
                                                          None)  # Accuracy of feature set being tested

                if not previous_run == feature_to_use:
                    print(
                        f"Using feature(s) {sorted(feature_to_use)} accuracy is {round(accuracy * 100, 2)}%")  # Print what features are being used
        ...
        print(
            f"Feature set {sorted(feature_set_to_keep)} was the best, accuracy is {round(best_accuracy * 100, 2)}%")  # Print the best found features at this level
...
def main():
    print("Welcome to Sarik's Feature Search Algorithm.")
    ...
    if algorithm_type == "1":
        print(
            f"This dataset has {data.shape[1] - 1} features (not including the class attribute), with {data.shape[0]} instances.")
        features = list(range(1, data.shape[1]))
        print()
        print(
            f"Running nearest neighbor with all {data.shape[1] - 1} features, using “leaving - one - out” evaluation, I get an accuracy of {round(leave_one_out_cross_validation(data, features, None) * 100, 2)}%. ")
        print()
